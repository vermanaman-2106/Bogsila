...
export default function CoordScreen() {
  ...
  async function load() {
    try {
      ...
      setError(e?.message || 'Failed to fetch co-ord');
    } finally {
      setLoading(false);
    }
  }
  ...
  if (loading) {
    return (
      <View className="flex-1 items-center justify-center bg-black">
        ...
        <Text className="text-white mt-3">Loading Co-ord...</Text>
      </View>
    );
  }
  ...
  return (
    <View className="flex-1 bg-black px-5 pt-5">
      <Text className="text-white text-2xl font-semibold mb-4">Co-ord</Text>
      <FlatList
        data={items}
        numColumns={2}
