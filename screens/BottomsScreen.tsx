...
export default function BottomsScreen() {
  ...
  async function load() {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch(`${BASE_URL}/api/bottoms?limit=1000`);
      const json = await res.json();
      setItems(Array.isArray(json.items) ? json.items : []);
    } catch (e: any) {
      setError(e?.message || 'Failed to fetch bottoms');
    } finally {
      setLoading(false);
    }
  }
  ...
  if (loading) {
    return (
      <View className="flex-1 items-center justify-center bg-black">
        ...
        <Text className="text-white mt-3">Loading Bottoms...</Text>
      </View>
    );
  }
  ...
  return (
    <View className="flex-1 bg-black px-5 pt-5">
      <Text className="text-white text-2xl font-semibold mb-4">Bottoms</Text>
      <FlatList
        data={items}
        numColumns={2}
